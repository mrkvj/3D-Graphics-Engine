...
static void _log_and_fail(GLint handle, const char*adverb, const char *path, void (*getlog)(GLuint, GLsizei, GLsizei *, GLchar*), void (*getiv)(GLuint, GLenum, GLint *)) {
    GLint loglen;
...
static GLint _compile(char *path, GLenum type) {
    ...
    f = fopen(path, "rb");
    if (f==NULL) {
        fprintf(stderr, "ERROR: Loading shader %s failed.\n", path);
        exit(1);
    }
